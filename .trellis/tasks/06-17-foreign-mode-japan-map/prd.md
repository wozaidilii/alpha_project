...
- 用户要求增加邮箱登录和校验码，并在数据库中记录各个用户的行为数据。
- Web 登录采用两步 tRPC 流程：先请求邮箱验证码，再提交 6 位验证码换取 `PlayerSession`。
...
- 旧的邮箱验证码账号如果已有 `players.email` 但没有 `password_hash`，注册时应允许补设唯一用户名和密码；重置密码也应能给这类邮箱账号发送验证码并设置密码。
...
- 用户反馈无痕窗口中 Google 邮箱登录只显示通用失败提示，无法判断是 OAuth 配置、state cookie、profile 拉取还是数据库写入问题。
- Google OAuth callback 需要把失败阶段映射为可区分的 `google_*` 错误码，并在登录页显示可操作的排障提示。
- 首页需要增加登录按钮，并保留现有中文、日文、英文三语切换。
- 首页选择的语言要通过 LocalStorage 和 `lang` query 传递到 `/game/anime`、登录回跳和结算页。
- 动漫题库生成脚本需要抽取原始 `details.i18n.ja/en`，public 题库保留中文根字段并提供日文、英文覆盖字段。
- 游戏页和结算页的按钮、状态、线索标题、答案、分享文案、分数和错误提示都应跟随首页语言设置。

## Scope Update: 多人对战同步重构

- 用户反馈对战模式经常出现两边不同步，并要求对战模式适配当前二次元猜谜视觉风格、支持 2 人以上同时对战。
- 对战房间最大人数提升为 `BATTLE_MAX_PLAYERS`，当前值为 8；最少 2 人即可开始。
- 对战中不再只依赖 Pusher 事件推进状态；提交猜测、回合结果、准备状态、下一轮和结束状态都写回 `/api/battle/rooms/[roomId]` 快照。
- 客户端在大厅、游玩、结算和结束阶段都可以通过轮询快照恢复状态，Pusher 只作为即时通知通道。
- 多人伤害规则改为按本轮最高分计算，低于最高分的玩家按分差扣血，最高分并列者不扣血。
- 对战 UI 改为显示所有玩家血条、提交状态和多人排名，不再使用固定 1v1 的 `VS` 结构。
...
- [ ] 2 人以上加入房间后，房主点击开始，所有玩家都进入同一局游戏。
- [ ] 对战进行中刷新或错过 Pusher 事件后，客户端可通过房间快照恢复提交、结果、准备和结束状态。
- [ ] 多人对战结算页能展示所有玩家排名和 HP。
...
- [ ] Google 登录失败时按 state/config/token/profile/database/unknown 显示可区分错误提示。
...
- [ ] `/` 提供登录按钮，并把当前语言作为登录后的回跳上下文。
- [ ] 动漫 public 题库包含日文和英文覆盖文本，不只包含中文字段。
- [ ] `/game/anime` 和结算页文案、题目线索、答案、分享文案跟随首页语言设置。
